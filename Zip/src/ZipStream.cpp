...
// ZipStream.cpp
//
// $Id: //poco/Main/Zip/src/ZipStream.cpp#10 $
//
// Library: Zip
// Package: Zip
// Module:  ZipStream
	...
	_pHeader(0)
{
	...
	if (fileEntry.getCompressionMethod() == ZipCommon::CM_DEFLATE)
	{
		...
		if (fileEntry.searchCRCAndSizesAfterData())
			_ptrHelper = new AutoDetectInputStream(istr, init, crc);
		else
	...
	{
		if (fileEntry.searchCRCAndSizesAfterData())
			_ptrBuf = new AutoDetectInputStream(istr);
		else
	...
	else
	{
		if (_crc32.checksum() != _expectedCrc32)
		{
			...
			else
			{
				...
				Poco::UInt32 size = nfo.getFullHeaderSize();
				const char* rawHeader = nfo.getRawHeader();
				for (Poco::UInt32 i = size-1; i >= 0; --i)
					_pIstr->putback(rawHeader[i]);
				if (_crc32.checksum() != nfo.getCRC32())
					throw ZipException("CRC failure");
			}
		}
	}
