...
// ZipTest.cpp
//
// $Id: //poco/Main/Zip/testsuite/src/ZipTest.cpp#6 $
//
...
std::string ZipTest::getTestFile()
{
	Poco::Path root;
	root.makeAbsolute();
	Poco::Path result;
	...
	result.setFileName("test.zip");
	Poco::File aFile(result.toString());
	if (!aFile.exists() || (aFile.exists() && !aFile.isFile()))
		throw Poco::FileNotFoundException("Didn't find test.zip file");
...
{
	Poco::URI uri("http://www.cio.gv.at/onlineservices/datacenter/moa-spss/data/customer1.zip");
