...
// Address.h
//
// $Id: //poco/svn/Net/include/Poco/Net/Address.h#2 $
//
// Library: Net
// Package: NetCore
// Module:  Address
...
#ifndef Sockets_Address_INCLUDED
#define Sockets_Address_INCLUDED
...
public:
	...
	Address();
		/// Creates a wildcard (zero) IPv4 Address.

	Address(const Address& addr);
		/// Creates an Address by copying another one.

	explicit Address(Family family);
		/// Creates a wildcard (zero) Address for the
		/// given address family.

	explicit Address(const std::string& addr);
		/// Creates an Address from the string containing
	...
	Address(const std::string& addr, Family family);
		/// Creates an Address from the string containing
		/// an IP address in presentation format (dotted decimal
		/// for IPv4, hex string for IPv6).

	Address(const void* addr, poco_socklen_t length);
		/// Creates an Address from a native internet address.
	...
	void mask(const Address& mask, const Address& set);
		/// Masks the IP address using the given netmask, which is usually
		/// a IPv4 subnet mask. Only supported for IPv4 addresses.
		///
		/// The new address is (address & mask) | (set & ~mask).
		
	static Address parse(const std::string& addr);
		/// Creates an Address from the string containing
		/// an IP address in presentation format (dotted decimal
		/// for IPv4, hex string for IPv6).
		/// 
	...
	static bool tryParse(const std::string& addr, Address& result);
		/// Tries to interpret the given address string as an
		/// IP address in presentation format (dotted decimal
		/// for IPv4, hex string for IPv6).
		///
		/// Returns true and stores the Address in result if the
...
#endif // Sockets_Address_INCLUDED
