...
#include "Poco/Sockets/NetworkInterface.h"
...
public:
	NetworkInterfaceImpl();
	NetworkInterfaceImpl(const std::string& name, const std::string& displayName, const Address& address, int index = -1);
	NetworkInterfaceImpl(const std::string& name, const std::string& displayName, const Address& address, const Address& subnetMask, const Address& broadcastAddress, int index = -1);

	int index() const;		
	const std::string& name() const;
	const std::string& displayName() const;
	const Address& address() const;
	const Address& subnetMask() const;
	const Address& broadcastAddress() const;
...
NetworkInterfaceImpl::NetworkInterfaceImpl(const std::string& name, const std::string& displayName, const Address& address, int index):
	_name(name),
	...
	_index(index)
{
	...
	if (index == -1) // IPv4
	{
		...
		DatagramSocket ds(Address::IPv4);
		ds.impl()->ioctl(SIOCGIFNETMASK, &ifr);
		if (ifr.ifr_addr.sa_family == AF_INET)
			_subnetMask = Address(&reinterpret_cast<const struct sockaddr_in*>(&ifr.ifr_addr)->sin_addr, sizeof(struct in_addr));
		if (!address.isLoopback())
		{
			try
			{
				...
				if (ifr.ifr_addr.sa_family == AF_INET)
					_broadcastAddress = Address(&reinterpret_cast<const struct sockaddr_in*>(&ifr.ifr_addr)->sin_addr, sizeof(struct in_addr));
			}
			catch (...)
			{
			}
...
NetworkInterfaceImpl::NetworkInterfaceImpl(const std::string& name, const std::string& displayName, const Address& address, const Address& subnetMask, const Address& broadcastAddress, int index):
	_name(name),
	_displayName(displayName),
	_address(address),
...
inline const Address& NetworkInterfaceImpl::address() const
{
	return _address;
}


inline const Address& NetworkInterfaceImpl::subnetMask() const
{
	return _subnetMask;
}


inline const Address& NetworkInterfaceImpl::broadcastAddress() const
{
...
NetworkInterface::NetworkInterface(const std::string& name, const std::string& displayName, const Address& address, int index):
	_pImpl(new NetworkInterfaceImpl(name, displayName, address, index))
{
}


NetworkInterface::NetworkInterface(const std::string& name, const std::string& displayName, const Address& address, const Address& subnetMask, const Address& broadcastAddress, int index):
	_pImpl(new NetworkInterfaceImpl(name, displayName, address, subnetMask, broadcastAddress, index))
{
}


NetworkInterface::NetworkInterface(const std::string& name, const Address& address, int index):
	_pImpl(new NetworkInterfaceImpl(name, name, address, index))
{
}


NetworkInterface::NetworkInterface(const std::string& name, const Address& address, const Address& subnetMask, const Address& broadcastAddress, int index):
...
const Address& NetworkInterface::address() const
{
	return _pImpl->address();
}


const Address& NetworkInterface::subnetMask() const
{
	return _pImpl->subnetMask();
}


const Address& NetworkInterface::broadcastAddress() const
{
...
NetworkInterface NetworkInterface::forName(const std::string& name, bool requireIPv6)
{
	...
	struct ifreq ifr;
	std::strncpy(ifr.ifr_name, name.c_str(), IFNAMSIZ);
	DatagramSocket ds(requireIPv6 ? Address::IPv6 : Address::IPv4);
	ds.impl()->ioctl(SIOCGIFADDR, &ifr);
	Address addr;
#if defined(POCO_HAVE_IPv6)
	if (ifr.ifr_addr.sa_family == AF_INET)
		addr = Address(&reinterpret_cast<const struct sockaddr_in*>(&ifr.ifr_addr)->sin_addr, sizeof(struct in_addr));
	else if (ifr.ifr_addr.sa_family == AF_INET6)
		addr = Address(&reinterpret_cast<const struct sockaddr_in6*>(&ifr.ifr_addr)->sin6_addr, sizeof(struct in6_addr));
	else
...
NetworkInterface NetworkInterface::forAddress(const Address& addr)
{
	NetworkInterfaceList ifs = list();
	for (NetworkInterfaceList::const_iterator it = ifs.begin(); it != ifs.end(); ++it)
	{
...
NetworkInterface::NetworkInterfaceList NetworkInterface::list()
{
	...
	{
		throw SocketException("cannot get network adapter list");
	}
	try
	{
		if (GetAdaptersAddresses(AF_INET6, 0, 0, pAdapterAddresses, &addrLen) == NO_ERROR) 
		{
			...
			while (pAddress) 
			{
				...
				{
					Address addr(pAddress->FirstUnicastAddress->Address.lpSockaddr, pAddress->FirstUnicastAddress->Address.iSockaddrLength);
		...
		else throw SocketException("cannot get network adapter list");
	}
	...
	result.push_back(NetworkInterface("Loopback", "Loopback Interface", Address("127.0.0.1"), Address("255.0.0.0"), Address(), -1));
	...
	try
	{
		if (GetAdaptersInfo(pAdapterInfo, &infoLen) == NO_ERROR) 
		{
			...
			while (pInfo) 
			{
				Address address(std::string(pInfo->IpAddressList.IpAddress.String));
				if (!address.isWildcard()) // only return interfaces that have an address assigned.
				{
					Address subnetMask(std::string(pInfo->IpAddressList.IpMask.String));
					Address broadcastAddress(address);
					broadcastAddress.mask(subnetMask, Address("255.255.255.255"));
...
NetworkInterface::NetworkInterfaceList NetworkInterface::list()
{
	...
	if (rc) throw SocketException("cannot get network adapter list");
	...
	for (struct ifaddrs* ifap = ifaphead; ifap; ifap = ifap->ifa_next) 
	{
		if (ifap->ifa_addr)
		{
			if (ifap->ifa_addr->sa_family == AF_INET)
			{
				...
				Address addr(&reinterpret_cast<struct sockaddr_in*>(ifap->ifa_addr)->sin_addr, sizeof(struct in_addr));
				Address subnetMask(&reinterpret_cast<struct sockaddr_in*>(ifap->ifa_netmask)->sin_addr, sizeof(struct in_addr));
				Address broadcastAddr;
				if (ifap->ifa_flags & IFF_BROADCAST)
					broadcastAddr = Address(&reinterpret_cast<struct sockaddr_in*>(ifap->ifa_dstaddr)->sin_addr, sizeof(struct in_addr));
				result.push_back(NetworkInterface(name, name, addr, subnetMask, broadcastAddr));
			}
#if defined(POCO_HAVE_IPv6)
			else if (ifap->ifa_addr->sa_family == AF_INET6)
			{
				Address addr(&reinterpret_cast<struct sockaddr_in6*>(ifap->ifa_addr)->sin6_addr, sizeof(struct in6_addr));
				std::string name(ifap->ifa_name);
