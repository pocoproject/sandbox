...
// DNS.cpp
//
// $Id: //poco/Main/Sockets/src/DNS.cpp#9 $
//
// Library: Sockets
// Package: Address
// Module:  DNS
...
HostEntry DNS::hostByName(const std::string& hostname)
{
	...
	throw SocketException(); // to silence compiler
}


HostEntry DNS::hostByAddress(const Address& address)
{
	FastMutex::ScopedLock lock(_mutex);
...
{
	Address ip;
	if (Address::tryParse(address, ip))
...
Address DNS::resolveOne(const std::string& address)
{
...
void DNS::error(int code, const std::string& arg)
{
	...
	case POCO_ESYSNOTREADY:
		throw SocketException("Net subsystem not ready");
	case POCO_ENOTINIT:
		throw SocketException("Net subsystem not initialized");
