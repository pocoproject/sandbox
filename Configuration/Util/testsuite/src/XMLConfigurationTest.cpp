...
void XMLConfigurationTest::testRemove()
{
	static const std::string xmlFile = "<config>"
		"   <prop1>value1</prop1>"
		"   <prop2>value2</prop2>"
		"   <prop3>"
		"       <prop4 attr='value3'/>"
	...
	pConf->setString("prop1", "bar");
	assert (pConf->hasProperty("prop1"));

	pConf->removeProperty("prop1");
	assert (!pConf->hasProperty("prop1"));

	try
	{
		pConf->setString("", "bar");
		fail("empty key - must throw");
...
CppUnit::Test* XMLConfigurationTest::suite()
{
	...
	CppUnit_addTest(pSuite, XMLConfigurationTest, testRemove);
