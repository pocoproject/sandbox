...
// SessionImpl.h
//
// $Id: //poco/1.4/Data/PgSQL/include/Poco/Data/PgSQL/SessionImpl.h#1 $
//
// Library: Data
// Package: PgSQL
// Module:  SessionImpl
...
#ifndef Data_PgSQL_SessionImpl_INCLUDED
#define Data_PgSQL_SessionImpl_INCLUDED
...
class PgSQL_API SessionImpl: public Poco::Data::AbstractSessionImpl<SessionImpl>
	/// Implements SessionImpl interface
{
public:
	static const std::string PgSQL_READ_UNCOMMITTED;
	static const std::string PgSQL_READ_COMMITTED;
	static const std::string PgSQL_REPEATABLE_READ;
	static const std::string PgSQL_SERIALIZABLE;
	...
	Poco::Data::StatementImpl* createStatementImpl();
		/// Returns an PgSQL StatementImpl
...
private:
	...
	template <typename T>
	inline T& getValue(PgSQL_BIND* pResult, T& val)
	{
		return val = *((T*) pResult->_paramValue.data());
	}
		...
		/// Limited to one setting at a time.
	{
		...
		metadata.reset();
		ex.prepare(Poco::format("SELECT @@%s", name), 0, 0);
		metadata.init(ex);
		...
		ex.execute(0,0);
		ex.fetch();
		PgSQL_BIND* pResult = metadata.row();
...
{
#if 0
	return Poco::Any(Poco::UInt64(PgSQL_insert_id(_handle)));
#endif
}
...
#if 0
template <>
inline std::string& SessionImpl::getValue(PgSQL_BIND* pResult, std::string& val)
{
...
#endif // Data_PgSQL_SessionImpl_INCLUDED
